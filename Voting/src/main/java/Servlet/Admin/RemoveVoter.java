 ...
 */
@WebServlet("/RemoveVoter")
public class RemoveVoter extends HttpServlet {
...
//		JSONObject jsonObject = (JSONObject) request.getAttribute("object");
		...
		try {
			isRemoved = adminManagement.removeVoters(Integer.parseInt(voterId));
	    ...
	    if(isRemoved) {
	        ...
	        try {
				...
				json.put("message", "The voter was removed");
			} 
	        catch (JSONException e) {
	        	logger.error("Admin : "+id+" "+ e.getMessage());
			}
			...
			try {
				...
				json.put("message", "Failed to remove voter");
			} catch (JSONException e) {
				logger.error("Admin : "+id+" "+ e.getMessage());
			}
			response.getWriter().write(json.toString());
